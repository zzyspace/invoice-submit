...
新增共用工作台入口 `/mini.html`，由源码 `mini.html` 构建为 `public/mini.html`，供浏览器和独立微信小程序外壳共同使用。公共表单沿用原路径，管理入口使用现有统一会话授权；没有新增业务登录或数据副本。构建、浏览器验证和双端发布方式见 [小程序入口说明](docs/mini-entry.md)。
